...
def analyze_snapshot(conn, ledger, snapshot, force_compute):
    cursor = conn.cursor()
    ...
    entries = None
    metrics_results = {}
    for metric_name in compute_functions.keys():
...
def get_output(ledger, year, metrics):
...
def analyze(ledgers, snapshots, force_compute, db_directories):
    output = []
    for ledger in ledgers:
        for snapshot in snapshots:
            ...
            metrics_values = analyze_snapshot(conn, ledger, snapshot, force_compute)
            output.append(get_output(ledger, year, metrics_values))
